...
/** ================== CONFIG & STYLES FIRST (prevents minified-undefined issues) ================== */
const API_URL =
...
/** ================== TINY, LOCAL COMPONENTS (renamed to avoid stale 'StatusBadge') ================== */
function StatusTag({ status }) {
  ...
  return <span style={{ background:sty.bg, color:sty.fg, padding:"2px 8px", borderRadius:999, fontSize:12, marginRight:6 }}>{sty.label}</span>;
}

function OwnerTag({ ownerStatus }) {
          ...
          <div key={r.id} style={reqRow}>
            <div style={{ flex:1 }}>
              ...
              <div style={{ marginTop:4 }}>
                ...
                <OwnerTag ownerStatus={r.owner_status} />
...
function SupervisorAdmin(props) {
  const {
    ...
    lastInviteLink, fetchUsers, users,
    resetPasswordFor, toggleActiveFor, deleteUser, loading
  } = props;
...
export default function App() {
  ...
  async function ownerInviteUser() {
  ...
  async function supervisorInviteEmployee() {
  ...
  async function toggleActiveFor(u) {
    setError(""); setLoading(true);
    try {
